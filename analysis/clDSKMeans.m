function clDSKMeans(output,output_figs)
    ...
    for i = length(files):-1:1
        ...
        str = fullfile(output,'clustering_subsegments_skm',tmp);
        if ~exist(str,'dir')
            mkdir(str)
        end
        if isempty(dir(fullfile(str,'*.mat')))
            ...
            cl_results = struct('x',[],'initC',[],'pcs',[],'idx',{},'centroids',{},'w',{},...
                'wcd',{},'bcd',{},'Silh2',{},'Si',{},'Silh2w',{},'Siw',{},'cm',{});
            ...
            for ii = 1:length(pcs)
                ...
                ss = 0.1:0.2:sqrt(size(x,2));
                for jj = 1:length(ks)
                    for jjj = 1:length(ss)
                        %Clustering algorith
                        C = dkmpp_init(x,ks(jj));
                        [idx,centroids,w] = pcskmeans(x,ks(jj),ss(jjj),[],x(C,:));
                        initC = x(C,:);
                        %Clustering metrics
                        ...
                        cl_results(jj,ii).idx = [cl_results(jj,ii).idx, {idx}];
                        cl_results(jj,ii).centroids = [cl_results(jj,ii).centroids, {centroids}];
                        cl_results(jj,ii).w = [cl_results(jj,ii).w, {w}];
                        cl_results(jj,ii).wcd = [cl_results(jj,ii).wcd, {wcd}];
                        cl_results(jj,ii).bcd = [cl_results(jj,ii).bcd, {bcd}];
                        cl_results(jj,ii).Silh2 = [cl_results(jj,ii).Silh2, {Silh2}];
                        cl_results(jj,ii).Silh2w = [cl_results(jj,ii).Silh2w, {Silh2w}];
                        cl_results(jj,ii).Si = [cl_results(jj,ii).Si, {Si}];
                        cl_results(jj,ii).Siw = [cl_results(jj,ii).Siw, {Siw}];
                        cl_results(jj,ii).cm = [cl_results(jj,ii).cm, {cm}];
                    end
                end
            end
            %Save
        ...
        for zz = 1:length(ss) 
            ...
            for I = 1:n
                for j = 1:m
                    ...
                    tmp{zz}(tmp{zz}==1) = [];
                    max_corr(I,j) = 100*max(tmp{zz});
                    tmp = cl_results(I,j).idx{zz};
