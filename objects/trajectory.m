classdef trajectory < handle
    ...
    methods
        % constructor
        function traj = trajectory(pts, set, track, group, id, trial, session, segment, off, starti, trial_type, props)
            traj.points = pts;                       
            traj.set = set;
            traj.track = track;
            ...
            if nargin >= 12
                traj.properties_ = props;           
            else
                traj.properties_ = containers.Map('KeyType', 'char', 'ValueType', 'any');        
            end
        end
        ...
        function [ prop ] = all_properties(inst)
            prop = inst.properties_;
        end
        ...
        function [ segment ] = sub_segment(traj, beg, len)
            ...
            segment = trajectory(pts, traj.set, traj.track, traj.group, traj.id, traj.trial, traj.session, 0, traj.offset + beg, traj.start_index + starti - 1, traj.trial_type, traj.properties_);   
        end  
         ...
         function [ segment ] = sub_segment_time(traj, toff, dt)
            ...
            segment = trajectory(pts, traj.set, traj.track, traj.group, traj.id, traj.trial, traj.session, 0, traj.offset + dist, traj.start_index + starti - 1, traj.trial_type, traj.properties_);   
        end 
